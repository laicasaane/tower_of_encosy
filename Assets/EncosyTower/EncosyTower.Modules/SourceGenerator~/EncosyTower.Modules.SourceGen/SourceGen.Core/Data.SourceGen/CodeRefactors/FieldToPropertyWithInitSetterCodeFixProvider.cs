...
{
    [ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(FieldToPropertyWithInitSetterCodeFixProvider)), Shared]
    internal class FieldToPropertyWithInitSetterCodeFixProvider : CodeFixProvider
    {
        ...
        public sealed override async Task RegisterCodeFixesAsync(CodeFixContext context)
        {
            ...
            context.RegisterCodeFix(
                CodeAction.Create(
                      title: "Replace field by property (with init-only setter)"
                    , createChangedSolution: c => MakePropertyAsync(context.Document, declaration, c)
                    , equivalenceKey: "Replace field by property (with init-only setter)"
                )
                , diagnostic
            );
        }
        ...
        )
        {
            ...
            propDecl = propDecl.WithAccessorList(SyntaxFactory.AccessorList(SyntaxFactory.List(new[] { getDecl, initDecl })))
                .WithTrailingTrivia(fieldDecl.GetTrailingTrivia());
