...
namespace EncosyTower.SourceGen.Generators.Mvvm.InternalUnions
{
    partial class InternalUnionDeclaration
    {
            ...
            foreach (var typeRef in RefTypeRefs)
            {
                try
                {
                    ...
                    context.OutputSource(
                        ...
                        , null
                        , syntax
                        , GetSourceForInternalUnionForRefType(typeRef, compilation.Assembly.Name)
                        , hintName
                        , sourceFilePath
                    );
                }
        ...
        private static string GetSourceForInternalUnionForRefType(TypeRef typeRef, string assemblyName)
        {
            ...
            p.PrintLine($"namespace EncosyTower.Unions.__Internal.{assemblyName.ToValidIdentifier()}");
            p.OpenScope();
            {
