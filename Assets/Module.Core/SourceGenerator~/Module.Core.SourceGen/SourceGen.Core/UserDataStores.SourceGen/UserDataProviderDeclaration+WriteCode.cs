...
namespace Module.Core.UserDataStores.SourceGen
{
    partial class UserDataProviderDeclaration
    {
        ...
        )
        {
            ...
            p.OpenScope();
            {
                ...
                foreach (var kv in loopMap)
                {
                    ...
                    p.PrintBeginLine("global::Module.Core.Logging.DevLogger.LogError(")
        ...
        private void WriteDataStorageClass(ref Printer p, ReadOnlySpan<StorageDefinition> defs)
        {
            ...
            p.OpenScope();
            {
                p.PrintLine("private readonly global::Module.Core.Encryption.RijndaelEncryption _encryption;");
                p.PrintEndLine();
                ...
                foreach (var def in defs)
                {
                    p.PrintBeginLine("public global::Module.Core.UserDataStores.UserDataStorage<")
        ...
        private static void WriteCollectionsClass(ref Printer p, ReadOnlySpan<StorageDefinition> defs)
        {
            ...
            p.OpenScope();
            {
                foreach (var def in defs)
                {
                    ...
                    p.PrintBeginLine("public static readonly global::Module.Core.NameKeys.NameKey<string> ").Print(name)
                        .Print(" = global::Module.Core.NameKeys.NameToKey.Get<string>(nameof(").Print(name).PrintEndLine("));")
                        .PrintEndLine();
                }
            }
            p.CloseScope();
