...
namespace Module.Core.Editor
{
    public class EditorIcons : EditorWindow
    {
        ...
        private static bool s_darkPreview = true;
        ...
        private static void SaveAllIcons()
        {
            var folderpath = EditorUtility.SaveFolderPanel("", "", "");

            try
            {
                var icons = s_icons.AsSpan();
        ...
        private void OnEnable()
        {
            ...
            {
                var icoContent = GetIcon(tex.name);
        ...
        private void OnGUI()
        {
            ...
            using (var scope = new GUILayout.ScrollViewScope(_scroll))
            {
                ...
                List<GUIContent> iconList = DoSearch
                    ? s_iconContentListAll.Where(x => x.tooltip.ToLower().Contains(_search.ToLower())).ToList()
                    : s_viewBigIcons ? s_iconContentListBig : s_iconContentListSmall;
                ...
                while (index < iconList.Count)
                {
                    using (new GUILayout.HorizontalScope())
                    {
                        ...
                        for (var i = 0; i < gridW; ++i)
                        {
                            ...
                            if (GUILayout.Button(icon, s_iconButtonStyle, GUILayout.Width(_buttonSize), GUILayout.Height(_buttonSize)))
                            {
            ...
            using (new GUILayout.HorizontalScope(EditorStyles.helpBox, GUILayout.MaxHeight(s_viewBigIcons ? 140 : 120)))
            {
                using (new GUILayout.VerticalScope(GUILayout.Width(130)))
                {
                    ...
                    GUILayout.Button(
                          s_iconSelected
                        , s_darkPreview ? s_iconPreviewBlack : s_iconPreviewWhite
                        , GUILayout.Width(128)
                        , GUILayout.Height(s_viewBigIcons ? 128 : 40)
                    );
        ...
        private static void InitIcons()
        {
            ...
            var iconContentListBig = s_iconContentListBig = new List<GUIContent>();
            ...
            for (var i = 0; i < iconsLength; i++)
            {
                ...
                iconContentListAll.Add(content);

                if (!(content.image.width <= 36 || content.image.height <= 36))
                {
                    iconContentListBig.Add(content);
                }
                else
                {
                    iconContentListSmall.Add(content);
                }
            }
        }
