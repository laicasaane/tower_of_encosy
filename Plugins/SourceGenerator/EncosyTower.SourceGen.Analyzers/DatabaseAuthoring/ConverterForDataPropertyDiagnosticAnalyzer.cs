...
{
    /// <summary>
    /// Reports <see cref="MisplacedConverterAttribute"/> when a
    /// <c>ConverterForDataPropertyAttribute</c> is placed on a type that is not annotated with
    /// <c>AuthorDatabaseAttribute</c>.
    /// </summary>
    [DiagnosticAnalyzer(LanguageNames.CSharp)]
    public sealed class ConverterForDataPropertyDiagnosticAnalyzer : DiagnosticAnalyzer
    {
        private const string AUTHOR_DATABASE_ATTRIBUTE = "global::EncosyTower.Databases.Authoring.AuthorDatabaseAttribute";
        private const string CONVERTER_FOR_DATA_PROPERTY_ATTRIBUTE = "global::EncosyTower.Databases.Authoring.ConverterForDataPropertyAttribute";

        public static readonly DiagnosticDescriptor MisplacedConverterAttribute = new DiagnosticDescriptor(
              id: "SG_AUTHOR_DATABASE_0080"
        ...
        private static void Analyze(SymbolAnalysisContext context)
        {
            ...
            foreach (var attrib in symbol.GetAttributes())
            {
                ...
                if (attrib.AttributeClass?.HasFullName(CONVERTER_FOR_DATA_PROPERTY_ATTRIBUTE, token) != true)
                {
                    continue;
                }
