...
namespace EncosyTower.ResourceKeys
{
    ...
    public readonly partial struct ResourceKeyError
    {
        ...
        readonly partial struct Error
        {
            ...
            public readonly partial record struct InvalidResourceKey(ResourceKey Key)
            {
                public string ToMessage(string prefix)
                {
            ...
            public readonly partial record struct LoadException(ResourceKey Key, Exception Exception)
            {
                public string ToMessage(string prefix)
                {
                    const string MSG1 = "Exception '";
                    const string MSG2 = "' occurred while loading the resource key '";
                    const string MSG3 = "'. ";

                    using var _ = StringBuilderPool.Rent(out var sb);
                    return InitString(sb, prefix).Append(MSG1).Append(Exception.GetType().Name).Append(MSG2)
                        .Append(Key.ToString()).Append(MSG3).Append(Exception).ToString();
                }
            }
